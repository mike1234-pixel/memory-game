...
const App: React.FC<AppProps> = (props: AppProps) => {
      ...
      setTimeout(() => {
        ...
        if (numberOfPlayers !== 1) {
          if (allAreEqual(scores)) {
            ...
            alert(
              `Player ${
                scores.indexOf(lowest) + 1
              } wins by ${difference} points`
            );
          }
        }
  ...
  return (
    <div className="container">
      <h1>Memory Game</h1>
      <p>
        The aim of the game is to match up all the cards in the fewest turns
        possible. Good luck!
      </p>

      <h2 className={`player-turns ${gameStarted}`}>
        Player {scores.length + 1 !== numberOfPlayers + 1 && scores.length + 1}:
        Turns = {turns}
      </h2>
      <h3>select number of players</h3>
      <div>
        <button className="button button--round" onClick={removePlayer}>
          -
        </button>
        <span className="number-of-players">{numberOfPlayers}</span>
        <button className="button button--round" onClick={addPlayer}>
          +
        </button>
      </div>
      ...
      <div className={`cards ${gameStarted}`}>
        {cards.length > 0 ? (
          cards.map((card: CardI, i: number) => {
            const {
              id,
              url,
              matched,
            }: { id: string; url: string; matched: boolean } = card;
            ...
            return (
              <Card
                ...
                matched={matched}
                flipped={
                  matched || id + i === currentCardKey || id + i === prevCardKey
                } // if card is a matched card, or the last clicked card, it should be flipped
